...
from py_abm.fitness.abm import ABMFitness, entities
...
class DiscreteABMFitness(Fitness):
    ...
    def __init__(self,
                 ...
                 discrete_strategy: None | Callable[[np.ndarray],
                                                    np.ndarray] = None,
                 objectives: list[entities.Objective] | None = None,
        ...
        Args:
            ...
            discrete_strategy (None | Callable[[np.ndarray], 
                                        np.ndarray]): estratégia de discretização.
            ...
            sql_row_limit (int, optional): quantidade máxima de entradas que podem
                ser armazenadas no banco.

        """
        assert sql_file_path is not None
        if discrete_strategy is None:
            discrete_strategy = nearest_hundredth

        self._fn = ABMFitness(market=market,
                              optimization_type=optimization_type,
                              time_steps=time_steps,
                              mc_runs=mc_runs,
                              r_values_from_individual=r_values_from_individual,
                              objectives=objectives)
        # self._db = _SQLDatabase(sql_file_path,
        #                        sql_row_limit)
        # Desde 18/05/2023, a implementação padrão usa
        #   um banco de dados e memória.
...
class _InMemoryDatabase:
    ...
    def save(self,
             key: np.ndarray,
             value: np.ndarray,
             output: entities.ABMJarOutput) -> bool:
...
def output_to_bytes(output: entities.ABMJarOutput) -> bytes:
    """Transforma a saída da simulador em um objeto
    bytes através do pickle.

    Args:
        output (entities.ABMJarOutput): saída do ABM.
...
def output_from_bytes(value: bytes) -> entities.ABMJarOutput:
    ...
    Returns:
        entities.ABMJarOutput: saída do simulador.
    """
    return pickle.loads(value)
